    ...
    <Page size="A4" style={styles.page}>
      ...
      <View style={styles.section}>
        ...
        <Text style={styles.text}>
          Email: {orderData.userId?.email || "N/A"}
        </Text>
        <Text style={styles.text}>
          Phone: {orderData.userId?.contactNumber || "N/A"}
        </Text>
      </View>
      ...
      <Text style={styles.text}>
        Transaction ID: {orderData.paymentInfo.cardNumber}
      </Text>

      {/* Shipment Details in PDF */}
      {orderData.shipmentData && (
        <View style={styles.section}>
          <Text style={styles.heading}>Delivery Details:</Text>
          <Text style={styles.text}>
            Recipient: {orderData.shipmentData.fullName}
          </Text>
          <Text style={styles.text}>
            Mobile: {orderData.shipmentData.mobileNumber}
          </Text>
          <Text style={styles.text}>
            Address: {orderData.shipmentData.shippingAddress}
          </Text>
          <Text style={styles.text}>
            Type: {orderData.shipmentData.deliveryType}
          </Text>
        </View>
      )}
      ...
      <View style={styles.section}>
        ...
        >
          Grand Total: BDT {orderData.totalPrice}
        </Text>
      </View>
...
const PaymentSuccess = () => {
    ...
    const fetchOrderAndClearCart = async () => {
      try {
        ...
        if (res.ok) {
          ...
          // Fallback: Check location state if API fails (e.g., net error or edge case)
          console.warn("API fetch failed, checking location state...");
          if (location.state?.order) {
            setOrderData(location.state.order);
          } else {
            const data = await res.json(); // Valid JSON error response checks
  ...
  }, [tranId, currentUser, location.state]); // Added location.state to dependency array
        ...
        <div className="p-8">
          ...
          {orderData?.shipmentData && (
            <div className="mb-8 p-4 bg-blue-50 dark:bg-gray-700 rounded-lg">
              <h2 className="text-lg font-semibold text-gray-800 dark:text-white mb-3 border-b border-blue-200 dark:border-gray-600 pb-2">
              ...
              <div className="space-y-2 text-sm text-gray-700 dark:text-gray-300">
                <div className="flex flex-col sm:flex-row sm:justify-between">
                  ...
                  <span>{orderData.shipmentData.fullName}</span>
                </div>
                <div className="flex flex-col sm:flex-row sm:justify-between">
                  <span className="font-medium text-gray-600 dark:text-gray-400">
                    Mobile:
                  </span>
                  <span>{orderData.shipmentData.mobileNumber}</span>
                </div>
                <div className="flex flex-col sm:flex-row sm:justify-between">
                  <span className="font-medium text-gray-600 dark:text-gray-400">
                    Address:
                  ...
                  <span className="text-right max-w-xs">
                    {orderData.shipmentData.shippingAddress}
                  </span>
                </div>
                <div className="flex flex-col sm:flex-row sm:justify-between">
                  <span className="font-medium text-gray-600 dark:text-gray-400">
                  ...
                  <span>{orderData.shipmentData.deliveryType}</span>
                </div>
                {orderData.shipmentData.specialInstructions && (
                  <div className="flex flex-col mt-2 pt-2 border-t border-blue-100 dark:border-gray-600">
